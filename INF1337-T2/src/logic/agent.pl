...
rotateAgentTo(Next) :- curPosition(X,Y,_), retract(curPosition(X,Y,_)), assert(curPosition(X,Y,Next)), decrementCost(1), timesTurned(Value), retractall(timesTurned(_)), Newvalue is Value + 1, assert(timesTurned(Newvalue)), !.
...
walkTo_Basic(X, Y) :- 
                ...
                ((enemyCell(EEE,_, X, Y), decrementEnergy(EEE));1=1), retractall(timesTurned(_)), assert(timesTurned(0)), !.

walkTo(X, Y) :- curPosition(XX,YY,_), walkTo_Basic(X, Y), path_addNode(XX, YY), retractall(timesTurned(_)), assert(timesTurned(0)).

agent_walkTo_noAddPath(X, Y) :- walkTo_Basic(X, Y), retractall(timesTurned(_)), assert(timesTurned(0)).