...
agent_reset() :- retractall(curEnergy(_)), retractall(curCost(_)), retractall(curAmmo(_)), retractall(curPosition(_,_,_)),
			    retractall(visited(_,_)), startPoint(StartX, StartY),
...
agent_walkTo(X, Y) :- isWalkable(X, Y), curPosition(_, _, Position), retractall(curPosition(_, _, _)), assert(curPosition(X, Y, Position)), (visited(X, Y);assert(visited(X, Y))),
                      ...
                      ((enemyCell(EEE,_, X, Y), agent_decrementEnergy(EEE), assert(hasEnemy(X, Y)), retractall(mightHaveEnemy(X, Y)), perceptions_updateUncertainties());1=1), !.
