...
int PIDList[NUMPROCESSOS][2]; // Lista de processos (PID) e suas prioridades. O Programa pode ter até 1024 processos na fila

int segmentoNomePrograma; // segmento (shm) para disparo de novos processos no escalonador
int segmentoPrioridade; // segmento (shm) para prioridade de novos processos no escalonador
...
void adicionarProcesso(int signal) {	// Evento de adicionar processo ao escalonador
  ...
  int *prioridade; // Prioridade do novo processo (shm)
  int prioridadeArg; // Prioridade do novo processo
  ...
  for(i = 0; i < NUMPROCESSOS; i++) {
    if(PIDList[i][0] == -1) { // Local livre na lista para adicionar o processo
      ...
      if(pid != 0) {
        sprintf(msgCriacao, "Processo %s criado - PID: %d - Prioridade: %d", nomeProcessoArg, pid, prioridadeArg);
        logInfo(msgCriacao);

        PIDList[i][0] = pid;
        PIDList[i][1] = prioridadeArg;
      }
      else {
...
int obtemProcessoAExecutar() {
  int i;
  int menorProcesso = -1; // índice do menor processo na lista
  for(i = 0; i < NUMPROCESSOS; i++) {
    if(PIDList[i][0] != -1)
    {
      if(menorProcesso == -1 || PIDList[i][1] < PIDList[menorProcesso][1])
      {
        menorProcesso = i;
      }
    }
...
int main(int argc, char *argv[]) {
  ...
  segmentoPrioridade = atoi(argv[2]);
