...
#include "inet/linklayer/blemesh/BleMeshProtocolDissector.h"

namespace inet {

Register_Protocol_Dissector(&Protocol::blemesh, BleMeshProtocolDissector);
...
void BleMeshProtocolDissector::dissect(Packet *packet, const Protocol *protocol, ICallback& callback) const
{
    ...
    callback.startProtocolDataUnit(&Protocol::blemesh);
    callback.visitChunk(header, &Protocol::blemesh);
...
//    }
    callback.endProtocolDataUnit(&Protocol::blemesh);
}
