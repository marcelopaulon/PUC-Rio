...
int segmentoPrioridade; // segmento para prioridade de novos processos no escalonador
...
void criaProcessoEscalonador(int fd[2]) {
  char argNomePrograma[11], argPrioridade[11];
  
  segmentoNomePrograma = shmget(IPC_PRIVATE, sizeof(char) * 1024, IPC_CREAT|IPC_EXCL|S_IRUSR|S_IWUSR);
  segmentoPrioridade = shmget(IPC_PRIVATE, sizeof(int), IPC_CREAT|IPC_EXCL|S_IRUSR|S_IWUSR);
  ...
  if(pidEscalonador == 0) // escalonador
  {
    ...
    snprintf(argPrioridade, 11, "%d", segmentoPrioridade);
    char * execEscalonador[] = {"./escalonador", argNomePrograma, argPrioridade, NULL};
...
void adicionaProcessos() {
  ...
  while(read(fileExec,&buffer[counter],1) != 0) {
    if(buffer[counter] == '\n') {
      buffer[counter] = '\0';
      sscanf(buffer, "exec %s prioridade=%d", nomeProcesso, prioridade);
      printf("[INFO] Processo %s - prioridade=%d enviado ao escalonador\n", nomeProcesso, *prioridade);
      fflush(stdout);
