...
public class GameControl {
	...
	private void setPlayerTrackToPocketMoves() {
		// TODO Auto-generated method stub
		
	}

	private void setPlayerTrackToLaneMoves(int diceValue, PlayerColor currentPlayer) {
		...
		for(int i = 1; i <= 52; i++)
		{
			...
			if(origin.getPawnCount() > 0 && origin.getPawnsColor().equals(currentPlayer)) //Checa se há peças na casa da cor do jogador
			{
				...
				{
					if(i + j == lastSquareOfCurrentPlayer) //Se ele chega na última casa com um número inferior ao tirado do dado, ele entra na lane.
					{ 
						...
						try {
							action = new MoveFromTrackSquareToLaneAction(origin, board.getLane(currentPlayer), diceValue - j, moveFromTrackToLaneActionListener);
	...
	private boolean canMoveInsideTrack(int diceValue, PlayerColor currentPlayer) {
		...
		for(int i = 1; i <= 52; i++)
		{
			...
			if(origin.getPawnCount() > 0 && origin.getPawnsColor().equals(currentPlayer)) //Checa se há peças na casa da cor do jogador
			{
				...
				for(int j = 1; j < diceValue; j++){
					if(i + j == lastSquareOfCurrentPlayer) return false; //Se ele chega na última casa com um número inferior ao tirado do dado, ele entra na lane. Logo, não pode continuar na track.
				}
