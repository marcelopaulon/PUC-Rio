	...
	public void render(Graphics2D g2d)
	{
		for(int i = 1; i <= 4; i++){
			...
			if(pawnCount > 0)
			{
				...
				Ellipse2D.Double pawn = new Ellipse2D.Double(pawnX + (squareSize * 0.05), pawnY + (squareSize * 0.05), 0.9 * squareSize, 0.9 * squareSize);
				g2d.fill(pawn);
				...
				if(pawnCount > 1)
				{
					...
					g2d.drawString(String.format("%d", pawnCount), (float) (pawnX + (squareSize * 0.40)), (float) (pawnY + (squareSize * 0.63)));
				}
